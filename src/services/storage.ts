...
const KEY_ACTIVE_CATEGORY = "active_category_id"; // 选中的分类 id（跨会话保留）
...
let activeCategoryIdCache: number | null = null;
...
export async function initStorage(): Promise<void> {
    const [tokenRes, urlRes, userInfoRes, activeNotebookRes, activeCategoryRes] = await Promise.all([
        ...
        Preferences.get({ key: KEY_ACTIVE_CATEGORY }),
    ]);
    ...
    activeNotebookIdCache = activeNotebookRes.value
        ...
        : null;
    activeCategoryIdCache = activeCategoryRes.value
        ? Number(activeCategoryRes.value)
        : null;
}
...
// ========== 选中分类（跨会话保留） ==========
/** 同步读取选中的分类 id */
export function getActiveCategoryId(): number | null {
    return activeCategoryIdCache;
}

/** 异步写入选中的分类 id：写 preferences + 更新内存 */
export async function setActiveCategoryId(id: number): Promise<void> {
    activeCategoryIdCache = id;
    await Preferences.set({ key: KEY_ACTIVE_CATEGORY, value: String(id) });
}
...
export function clearAll(): void {
    ...
    void Preferences.remove({ key: KEY_ACTIVE_CATEGORY });
}
